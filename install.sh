...
# First, download our installation script to the expected location
echo "Preparing Finance Assistant installation script..."

# Create the directory if it doesn't exist
mkdir -p /tmp/community-scripts-install

# Download our installation script to the expected location
curl -fsSL https://raw.githubusercontent.com/chbarnhouse/finance-assistant/main/install/financeassistant.sh -o /tmp/community-scripts-install/financeassistant-install.sh

# Now source the community framework
...
# Override the build_container function to use our local script
function build_container() {
  # Call the original build_container function but replace the problematic curl command
...
EOF
  ...
  # Install Finance Assistant using our local script instead of the 404-prone community script
  msg_info "Installing Finance Assistant inside container $CTID..."
  lxc-attach -n "$CTID" -- bash -c "$(cat /tmp/community-scripts-install/financeassistant-install.sh)" $?
}
